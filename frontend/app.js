...
audio.addEventListener("error", () => {
  // The source CDN sometimes drops the connection mid-stream - skip forward instead of
  // leaving playback silently stuck.
  if (current) {
...
function toggleLike(post) {
  ...
  if (!master) return;
  fetch(`/api/posts/${post.message_id}/like`, { method: "POST" })
    .then((r) => {
      if (!r.ok) throw new Error("like failed");
      return r.json();
    })
    .then((data) => {
      master.liked = data.liked;
      renderPostList();
    })
    .catch(() => alert("Не удалось изменить лайк"));
}
...
function setTrackCategory(track, catId, checked) {
  ...
  fetch(`/api/tracks/${track.id}/categories/${catId}`, { method: checked ? "POST" : "DELETE" });
}
...
function togglePostCategory(post, catId, checked) {
  ...
  popoverNeedsRerender = true;
  fetch(`/api/posts/${post.message_id}/categories/${catId}`, { method: checked ? "POST" : "DELETE" });
}
...
function deleteCategory(catId) {
  ...
  fetch(`/api/categories/${catId}`, { method: "DELETE" }).then(() => {
    categories = categories.filter((c) => c.id !== catId);
    posts.forEach((p) => p.tracks.forEach((t) => (t.categories = t.categories.filter((c) => c !== catId))));
...
async function pollSyncStatus() {
  ...
  if (s.running) {
    syncStatus.textContent = s.total
      ? `Синхронизация: ${s.processed}/${s.total}, добавлено ${s.added}`
      : "Синхронизация: получаю список сообщений...";
  }
  await refreshPosts();
  if (!s.running) {
    ...
    syncStatus.textContent = s.error ? `Ошибка: ${s.error}` : `Готово, добавлено ${s.added}`;
  }
}
...
(async function init() {
  ...
  // The server auto-starts a sync on its own startup, so this will usually already be
  // running - just attach to it instead of waiting for a button click.
  const s = await fetch("/api/sync/status").then((r) => r.json());
  if (s.running) {
    syncBtn.disabled = true;
    startPolling();
